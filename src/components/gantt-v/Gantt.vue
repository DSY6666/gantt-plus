				...
				<div class="head">
					<div class="head-box">
						<GanttRow :style="{ transform: `translatex(-${transFormXValue}px)` }" class="transition-transForm">
							<GanttTimeBlock :target-time="_formatDate(item)" v-for="(item, index) in choiceTimeArr" :key="index"></GanttTimeBlock>
						</GanttRow>
				...
				<div class="content" ref="contentGrid">
					...
					<div class="gantt-grid" ref="box" @contextmenu.prevent="() => {}">
						...
						>
							<template >
								<el-tooltip
									v-for="(times, index) in rowBlocks(item[listKey])" 
									:key="`times-${index}`" 
									effect="dark" 
									:content="formateTime(times)" 
									placement="bottom">
									<div
										:style="_timeBlockStyle(times)"
							...
							<GanttBlock 
								class="time" 
								v-for="(choiceTimeArrItem, _) in choiceTimeArr" 
								:key="_"
								:item="item"
								:rowIndex="rowIndex"
								:choiceTimeArrItem="choiceTimeArrItem"
		...
		<el-dialog title="编辑时间块" :visible.sync="isEditBlock">
			<el-form v-if="editBlock" :model="editBlock" label-width="120px">
				...
				<el-form-item label="开始时间">
					<el-date-picker @change="onChangeStartTime" v-model="editBlock.startTime" type="datetime" placeholder="选择日期时间"></el-date-picker>
				</el-form-item>
				<el-form-item label="时间长度">
					<el-input-number v-model="editBlock.timeDiff" @change="onChangeStartTime" :min="1" :max="100" label="时间长度"></el-input-number>
				</el-form-item>
				<el-form-item label="结束时间"><el-date-picker readonly v-model="editBlock.endTime" type="datetime" placeholder="选择日期时间"></el-date-picker></el-form-item>
			</el-form>
...
export default {
	...
	data() {
		return {
			...
			// 选择的时间区间
			choiceTime: [new Date('2022-04-14 00:00:00'), new Date('2022-04-20 00:00:00')],
	...
	methods: {
		...
		onMouseDown(e, times, rowIndex) {
			...
			window.onmouseup = () => {
				...
				const startTime = new Date(this.choiceTime[0].getTime() + (parseInt(dom.style.left) * 3600000) / 16);
				const endTime = new Date(this._getTime(startTime) + (parseInt(dom.style.width) * 3600000) / 16);
